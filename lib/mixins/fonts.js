...
export default {
  initFonts(
    ...
    // Note: defaultFont parameter default is null, but when called from document.js
    // with options.font === undefined, it passes undefined, not null
    let fontToUse;
    if (defaultFont === undefined) {
      // If defaultFont is undefined (not provided), use first registered font
      fontToUse = registry.getDefaultFont();
      if (!fontToUse) {
        throw new Error(
          'No fonts registered. Use getFontRegistry().registerStandardFonts() to register fonts before creating documents.',
        );
      }
    } else if (defaultFont === null || defaultFont === false || defaultFont === '') {
      // If explicitly null/false/empty string, don't set a font
      fontToUse = null;
    } else if (typeof defaultFont === 'string') {
      // If defaultFont is a string, check if it's a standard font and verify it's registered
      // Only validate standard fonts - custom fonts will be handled by font factory
      if (registry.hasStandardFont(defaultFont)) {
        // It's a standard font, verify it's registered (should always be true if hasStandardFont returns true)
        if (!registry.hasStandardFont(defaultFont)) {
          throw new Error(
            `Default font '${defaultFont}' is not registered. Use getFontRegistry().registerStandardFonts() to register fonts.`,
          );
        }
      }
      // If it's not a standard font, allow it (will be handled as custom font by font factory)
      fontToUse = defaultFont;
    } else {
      // Fallback: use first registered font
      fontToUse = registry.getDefaultFont();
      if (!fontToUse) {
        throw new Error(
          'No fonts registered. Use getFontRegistry().registerStandardFonts() to register fonts before creating documents.',
    ...
    // Set the default font (only if fontToUse is not null/false/empty)
    if (fontToUse) {
      this.font(fontToUse, defaultFontFamily);
    }
  },
  ...
  font(src, family, size) {
    ...
    // check registered custom fonts in global registry if src is a string
  ...
  registerFont(name, fontData, family) {
    const registry = getFontRegistry();
    registry.registerCustomFont(name, fontData, family);
    return this;
  },
