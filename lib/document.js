...
import { getStream, getBuffer, init as initAdapters } from './adapters';
...
// Lazy class definition - creates the class when first needed
...
function getPDFDocumentClass() {
  if (!PDFDocumentClass) {
    ...
    class PDFDocumentImpl extends Stream.Readable {
  constructor(options = {}) {
    super(options);
  ...
  async end() {
    ...
    await this.endMarkings();

    if (this.subset) {
      await this.endSubset();
    }

    await this.endMetadata();

    await this._root.end();
    await this._root.data.Pages.end();
    await this._root.data.Names.end();
    await this.endAcroForm();

    if (this._root.data.ViewerPreferences) {
      await this._root.data.ViewerPreferences.end();
    }

    if (this._security) {
      await this._security.end();
    }
  ...
  toString() {
    ...
    // Apply mixins
    const mixin = (methods) => {
      Object.assign(PDFDocumentImpl.prototype, methods);
    };
    ...
    PDFDocumentImpl.LineWrapper = LineWrapper;

    PDFDocumentClass = PDFDocumentImpl;
  }

  return PDFDocumentClass;
}

// Export a Proxy that creates the class on first use
const PDFDocument = new Proxy(function PDFDocument(...args) {
  return new (getPDFDocumentClass())(...args);
}, {
  get(target, prop) {
    const PDFDoc = getPDFDocumentClass();
    if (prop in PDFDoc) {
      return PDFDoc[prop];
    }
    return target[prop];
...
// Export init function for adapter initialization
export function init(options) {
  initAdapters(options);
}

// Export getFontRegistry for font registration
